...
<main class="container container-main margin-container">
	<nav aria-label="breadcrumb">
	  <ol class="breadcrumb-holder">
	    ...
	    <li class="breadcrumb-name active" aria-current="page">Purchases</li>
	  </ol>
	</nav>
	<section class="content-holder bg-white">
		<h5 class="font-orange label-title">VAT RELIEF: PURCHASES</h5>
		<hr>
		<a href="/relief/download/purchases" class="btn btn-success">DOWNLOAD EXCEL FORMAT</a>
		<br><br>
		<form method="POST" action="/relief/purchase/{{$company->id}}" class="needs-validation" enctype="multipart/form-data">
		  	@csrf	
		  	@if (session('warning'))
			...
			<div class="form-row">
				<div class="form-group col-md-6">
					<label for="inputCity">Non-Creditable:</label>
					<input type="text" value="" name="non-creditable" class="form-control">
				</div>
				<div class="form-group col-md-6">
					<label for="inputDistrict">Creditable:</label>
					<input type="text" value="" name="creditable" class="form-control">
				</div>
			</div>
		    <div class="form-group">
		    ...
		    <a href="/relief/{{ $company->id }}/histories/purchase" class="btn btn-danger">Back</a>
		</form>
	</section>
</main>
@endsection